...
const MEMO_WRAPPER_INFO = 'React.memo() returns a new object value on every call.';
...
/**
 * Generate message to suggest moving the component out of the parent.
 * @param {String} parentName Name of the parent component (if known)
 * @returns {String}
 */
function generateSuggestionMoveComponent(parentName) {
  return `Move this component definition out of the parent component${parentName ? ` "${parentName}" ` : ' '}and pass data as props.`;
}

/**
 * Generate message to suggest converting the component into a render function.
 * @param {ASTNode} node The AST node
 * @return {string}
 */
function generateSuggestionRenderFunction(node) {
  // Handles components created in props.
  const propName = node.parent && (
    // Extracts "prop" from <Child prop={() => <div />} />.
    (node.parent.parent && node.parent.parent.type === 'JSXAttribute' && node.parent.parent.name.name)
    // Extracts "key" from <Child prop={{ key: () => <div />}} />.
    || (node.parent.type === 'Property' && node.parent.key.name)
  );
  if (propName) {
    const renderPropName = `render${propName[0].toUpperCase()}${propName.substring(1)}`;
    return `Rename the prop "${propName}" to "${renderPropName}", then call the prop as a function in the child component.`;
  }
  ...
  const componentName = (
    ...
    || (node.parent && node.parent.id && node.parent.id.name)
    // Generic fallback.
    || 'Component'
  );
  const renderComponentName = `render${componentName}`;
  return `Rename "${componentName}" to "${renderComponentName}", then replace <${componentName} /> with {${renderComponentName}()}.`;
}
...
/**
 * Check whether given node is a call to `React.memo(...)`.
 * @param {ASTNode} node The AST node
 * @return {boolean}
 */
function isMemoWrapper(node) {
  return node.type === 'CallExpression' && node.callee.property.name === 'memo';
}
...
module.exports = {
  ...
  create: Components.detect((context, components, utils) => {
    ...
    function isStatelessComponentReturningNull(node) {
      ...
      return component && component.type !== 'CallExpression' && !utils.isReturningJSX(component);
    }
    ...
    function validate(node) {
      ...
      if (parentComponent) {
        ...
        const messages = [
          ...
          `- ${generateSuggestionMoveComponent(parentName)}`,
          `- ${generateSuggestionRenderFunction(node)}`,
        ];

        // Add information about React.memo() behavior.
        if (isMemoWrapper(node)) {
          messages.push(MEMO_WRAPPER_INFO);
        }

        // Add information about allowAsProps option when component is declared inside prop
        if (isDeclaredInsideProps && !allowAsProps) {
          messages.push(COMPONENT_AS_PROPS_INFO);
        }

        report(context, messages.join('\n'), null, {
          node,
        });
      }
    }
